...
def compression_cmd(dname, save, seed, silent, timeout):
    ...
    with open('results/xgb_oc_calcs.txt', 'r') as file: #Load in the XGB classification results
        for line in file:
            if not line.startswith('{'):
                continue
    ...
    for key, result in results_horse_race.items():
        ...
        idx = 3
        assert result['refinements'][idx]['penalty'] == 'ours'

        at = veritas.AddTree.from_json(result['refinements'][idx]['model_json'])
    
        result['refinements'][idx]['oc_score_training'] = calculate_oc_score(x=dtrain.X.to_numpy(), y=dtrain.y.to_numpy(), at=at)
        result['refinements'][idx]['oc_score_test'] = calculate_oc_score(x=dtest.X.to_numpy(), y=dtest.y.to_numpy(), at=at)

        result['refinements'][idx]['observed_oc_space_training'] = calculate_observable_oc_space(x=dtrain.X.to_numpy(), at=at)
        result['refinements'][idx]['observed_oc_space_test'] = calculate_observable_oc_space(x=dtest.X.to_numpy(), at=at)

        result['oc_space'] = count_ocs(at, timeout=21600)

        result['refinements'][idx]['verifications'] = run_verification_tasks(at, dtest.X, dtest.y, timeout=21600, n=500)

        result['refinements'][idx]['bound_oc_space'] = bound_oc_space(at)
