...
class Compress:
    ...
    def compress_level(self, level):
        ...
        for target in range(self.nlv):
            ...
            for i, alpha_record in enumerate(alpha_search):
                ...
                # Prune trees based on the coefficients to calculate the observable oc space for this alpha value
            ...
            del xxtrain, yytrain, xxvalid, yyvalid, clf, alpha_search
            if 'best' in locals():
                del best
            import gc
            gc.collect()
    ...
    def fit_coefficients(self, clf, xxtrain, yytrain, xxvalid, yyvalid, alpha_record):
        ...
        num_removed = np.sum(np.abs(clf.coef_) < 1e-5) #TODO is this the correct way to calculate num_removed? Because it is not directly counting the number of leafs removed.
