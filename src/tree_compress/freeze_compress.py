...
class Compress:
    ...
    def optimize_fixed_prox_l1(self, X, y, fixed_idx,
        ...
        step = 1.0 / L
        lam = alpha / n  # per-sample scaling
        ...
        for k in range(1, max_iter+1):
            # gradient at z
            base, grad_w, grad_b = data_loss(z, b, c, Xf, y)
            # gradient step on data term
            w_next = z - step * grad_w
            # prox for L1 (exact soft-threshold)
            w_next = self.soft_threshold(w_next, step * lam)
            ...
            # check convergence
            if np.linalg.norm(w_next - w, ord=2) < tol and abs(b_next - b) < tol:
                w, b = w_next, b_next
                break
    ...
    def _optimize_fixed(self, X, y, fixed_idx, loss_fn, fit_intercept=True, x0=None, tol=1e-5):
        """
        Generic optimizer for logistic regression with fixed coefficients.
        - X: (n_samples, n_features)
        - y: array in {0,1}
        - fixed_idx: indices of fixed coefficients
        - fixed_w: values of fixed coefficients
        - loss_fn: function (w_free, b, c, Xf, y) -> (loss, grad_w, grad_b)
                where:
                    w_free: free coefficients vector
                    b: intercept scalar
                    c: per-sample fixed offset (X_fixed @ fixed_w)
                    Xf: reduced feature matrix with free features
                    y: labels {0,1}
        - fit_intercept: whether to optimize an intercept
        - x0: initial guess for free parameters (default zeros)
        """
        n, p = X.shape
        ...
        # constant offset from fixed weights
        c = X[:, fixed_idx] @ fixed_w if fixed_idx.size else np.zeros(n)
        Xf = X[:, free_idx] if free_idx.size else np.zeros((n, 0))
        # init
        d = free_idx.size + (1 if fit_intercept else 0)
        if x0 is None:
            x0 = np.zeros(d)

        def unpack(x):
            if fit_intercept:
                return x[:free_idx.size], x[-1]
            else:
                return x, 0.0
        ...
        def fun_and_grad(x):
            w_free, b = unpack(x)
            # print(loss_fn)
            loss, grad_w, grad_b = loss_fn(w_free, b, c, Xf, y)
            if fit_intercept:
        ...
        res = minimize(lambda x: fun_and_grad(x)[0],
                    x0, jac=lambda x: fun_and_grad(x)[1],
                    method="L-BFGS-B", tol=tol, options={"maxiter": 5000, "disp": True})

        w_free, b = unpack(res.x)
        w_full = np.zeros(p)
        w_full[free_idx] = w_free
        w_full[fixed_idx] = fixed_w

        return {"w": w_full, "intercept": b, "success": res.success, "fun": res.fun, "message": res.message}
    
    def _refine_predict(self, w, b, X):
        """
        Refine the prediction using the optimized weights and intercept.
        - w: optimized weights
        - b: intercept
        - X: input features
        """
        if self.is_regression():
            ...
            logits = X @ w + b
            return np.round(expit(logits)).astype(int)  # Ensure binary output for classification tasks
